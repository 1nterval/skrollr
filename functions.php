...
function skrollr_print_assets(){
	...
	if( (defined('CSS_DEBUG') && !CSS_DEBUG && file_exists(dirname(__FILE__).'/build/cssbuild.php')) ) {
		...
		wp_enqueue_style('skrollr-cssbuild', get_template_directory_uri().'/css/'.CSSBUILD.'.min.css', array('mediaelement'), null);
	} else {
		wp_enqueue_style('skrollr-bootstrap-grid', get_template_directory_uri().'/css/libs/bootstrap-grid.css', array(), '3.3.1');
		// stylesheets (follow SMACSS guidelines)
		wp_enqueue_style('skrollr-base', get_template_directory_uri().'/css/base.css', array('skrollr-bootstrap-grid', 'mediaelement'), $ver);
		wp_enqueue_style('skrollr-layout', get_template_directory_uri().'/css/layout.css', array('skrollr-base'), $ver);
		wp_enqueue_style('skrollr-module', get_template_directory_uri().'/css/module.css', array('skrollr-layout'), $ver);
		wp_enqueue_style('skrollr-state', get_template_directory_uri().'/css/state.css', array('skrollr-module'), $ver);
	...
	if(defined('SCRIPT_DEBUG') && !SCRIPT_DEBUG && file_exists(dirname(__FILE__).'/build/jsbuild.php')){
		...
		wp_enqueue_script('skrollr-main', get_template_directory_uri().'/js/'.JSBUILD.'.min.js', array('jquery', 'mediaelement'), null, true);
	} else {
